...
export function Header({ selectedId, onCustomerChange, customerList = [], onFilterChange, hasRecentData, onSearch = () => { } }: HeaderProps) {
    ...
    const getPageContext = () => {
        ...
        if (path.includes('weather-2')) return { title: 'Weather Impact', icon: <CloudRain className="h-6 w-6" /> };
        if (path.includes('weather')) return { title: 'Map', icon: <CloudRain className="h-6 w-6" /> };
